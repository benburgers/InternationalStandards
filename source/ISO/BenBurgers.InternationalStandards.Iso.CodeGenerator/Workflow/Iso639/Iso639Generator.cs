...
internal static class Iso639Generator
{
    ...
    internal static async Task GenerateAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        using var loggerScope = logger.BeginScope("ISO 639 codes");
        ...
        iso639Namespace.Comments.AddRange(CopyrightNotice);
        iso639Namespace.Imports.Add(new CodeNamespaceImport($"{Namespace}.Attributes"));
        iso639Namespace.Types.Add(Enum(models, options.Part3Effective));
    ...
    private static string NormalizeName(string name)
    {
        ...
        var parts = name.Split(" ").Select(p => CapitalizeFirstLetter(SanitizeRegex.Replace(p, "_"))).ToArray();
        return string.Join("", parts);
    }

    private static readonly CodeCommentStatementCollection CopyrightNotice =
        new(
    ...
    private static CodeCommentStatementCollection EnumDocumentation(DateTimeOffset iso639EffectiveDate) =>
        new(
            new CodeCommentStatement[]
            {
                ...
                new("<remarks>", true),
                new("<para>The <a href=\"http://www.infoterm.info/\">International Information Centre for Terminology (Infoterm)</a> is the sole authority of ISO 639-1 codes.<br />Only they can issue ISO 639-1 codes. This work is a derivation and enhancement and anything other than ISO 639-1 is the responsibility of the respective copyright holders.</para>", true),
                new("<para>The <a href=\"https://www.loc.gov/standards/iso639-2/\">Library of Congress</a> is the sole authority of ISO 639-2 codes.<br />Only they can issue ISO 639-2 codes. This work is a derivation and enhancement and anything other than ISO 639-2 is the responsibility of the respective copyright holders.</para>", true),
                new($"<para>The <a href=\"https://www.iso639-3.sil.org\">SIL International</a> is the sole authority of ISO 639-3 codes.<br />Only they can issue ISO 639-3 codes. This work is a derivation and enhancement and anything other than ISO 639-3 is the responsibility of the respective copyright holders.<br />The ISO 639-3 codes are effective from {iso639EffectiveDate:D}.</para>", true),
    ...
    private static CodeTypeDeclaration Enum(IEnumerable<Iso639Model> models, DateTimeOffset iso639EffectiveDate)
    {
        var typeDeclaration = new CodeTypeDeclaration("Iso639Code") { IsEnum = true };
        typeDeclaration.Comments.AddRange(EnumDocumentation(iso639EffectiveDate));
    ...
    private static IEnumerable<CodeTypeMember> EnumFields(IEnumerable<Iso639Model> models)
    {
        foreach (var model in models)
        {
            ...
            member.Comments.Add(new CodeCommentStatement("\t\t<listheader>", true));
            member.Comments.Add(new CodeCommentStatement("\t\t\t<term>Name</term><description>Value</description>", true));
            member.Comments.Add(new CodeCommentStatement("\t\t</listheader>", true));

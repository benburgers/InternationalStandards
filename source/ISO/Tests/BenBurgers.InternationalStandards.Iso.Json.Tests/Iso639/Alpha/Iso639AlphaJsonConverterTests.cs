...
public class Iso639AlphaJsonConverterTests
{
    ...
    [Fact(DisplayName = "ISO 639 Alpha JSON Converter works with JSON Serializer.")]
    public void SerializerTest()
    {
        // Arrange
        const string Expected = "{\"Iso639\":\"mtv\",\"Name\":\"Asaro\"}";
        var value = new { Iso639 = Iso639Code.Asaro_o, Name = "Asaro" };
        var options = new Iso639JsonConverterOptions(Iso639AlphaMode.Part3);
        var jsonConverter = new Iso639AlphaJsonConverter(options);
        using var stream = new MemoryStream();
        using var utf8JsonWriter = new Utf8JsonWriter(stream);
        var jsonSerializerOptions = new JsonSerializerOptions { Converters = { jsonConverter } };

        // Act
        var actual = JsonSerializer.Serialize(value, jsonSerializerOptions);
